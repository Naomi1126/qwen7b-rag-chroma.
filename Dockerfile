...
# 3) Virtualenv y toolchain básico
RUN python3.11 -m venv /opt/venv && \
    /opt/venv/bin/pip install --upgrade pip setuptools wheel

# 4) Torch con CUDA 12.1 (instalado aparte)
RUN /opt/venv/bin/pip install --no-cache-dir \
    --index-url https://download.pytorch.org/whl/cu121 \
    torch==2.4.1 torchvision==0.19.1

# 5) Open WebUI SIN dependencias (para que no intente instalar su propio torch)
RUN /opt/venv/bin/pip install --no-cache-dir --no-deps open-webui==0.3.25

# 6) Resto de dependencias del proyecto
